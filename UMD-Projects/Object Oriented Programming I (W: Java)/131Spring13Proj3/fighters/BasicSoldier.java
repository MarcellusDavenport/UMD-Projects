...
public class BasicSoldier {
	...
	public int getDirectionOfNearestFriend() {
		...
		//Assigns a random place holder value to tempDistance
		int tempDistance = battleField.getRows() + battleField.getCols() ;
		...
		for (int battleRow = 0; battleRow < battleField.getRows(); battleRow++) {
			for (int battleCol = 0; battleCol < battleField.getCols(); battleCol++) {
				...
				|| ((battleField.get(battleRow, battleCol) == BattleField.BLUE_TEAM) && (team == BattleField.BLUE_TEAM))) {
					...
					if ((getDistance(battleRow, battleCol) < tempDistance) && (getDistance(battleRow, battleCol) > 0)) {
						
						//Assigns distance of closer friend to temp
						//Assigns direction of closer  friend to currentDirection
						tempDistance = getDistance(battleRow, battleCol);
	...
	public int getDirectionOfNearestEnemy(int radius) {
			...
			for (int battleCol = 0; battleCol < battleField.getCols(); battleCol++) {
				...
				if (((battleField.get(battleRow,battleCol) == BattleField.RED_TEAM) && (team == BattleField.BLUE_TEAM))
					...
					if (getDistance(battleRow,battleCol) <= radius) {
						...
						if (getDistance(battleRow, battleCol) < tempDistance) {
		...
		//In other words: if an enemy was never found within the desired radius, return NEUTRAL
		if (tempDistance == 1000) {
			return NEUTRAL;
		}
		
		//Return direction of closest enemy within the desired radius
		return currentDirection;
	}
	...
	public void performMyTurn() {
		...
		//Checks to see if the soldier came across an enemy
		//Attacking is the highest priority. Moving is second. Doing nothing is third

...
public class Restaurant {
	...
	public Restaurant(String nameIn, int startingCash) {
		...
		//ensures that they stay immutable  
	...
	public SortedListOfImmutables getMenu() {
		return new SortedListOfImmutables(menu); //keeps menu immutable 
	}
	...
	public void addEntree(Entree entreeToAdd) {
		menu.add(entreeToAdd); //adds entree to menu
	...
	public SortedListOfImmutables getInventory() {
		return new SortedListOfImmutables(inventory); //keeps inventory immutable
	}
	...
	public boolean checkIfInInventory(Entree entree) {
		//returns true if the foodList in the items exist in the inventory
	...
	public boolean addShipmentToInventory(SortedListOfImmutables list) {
		if (list.getWholesaleCost() > cash) {
			...
			inventory.add(list); //adds list to inventory
			cash = cash - list.getWholesaleCost(); //cash decreases by cost
			return true;
		}
	}
	...
	public boolean placeOrder(Entree entree) {
		if (!(checkIfInInventory(entree))) {
			return false; //if the foodList in entree doesn't exist in the inventory
		} else {
			cash += entree.getRetailValue(); //cash increases by retail value
			//the foodList in the entree is removed from inventory
